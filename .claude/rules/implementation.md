...
### 画面遷移時のチラツキ防止（lazyredraw）

**問題**: view 遷移時（list → describe など）に、新しい window がマウントされてから render が実行されるまでの間に空のバッファが一瞬表示される。

**原因**:
1. `window.mount(new_win)` で空のバッファが表示される
2. render がデバウンス（100ms）されているため遅延がある
3. その間、ユーザーには空の画面が見える

**解決策**: `vim.o.lazyredraw` で中間描画を抑制し、即時 render してから一度だけ `redraw` する。

```lua
-- 画面遷移時のパターン
local function push_new_view(view_state)
  -- 1. 中間描画を抑制
  local lazyredraw_was = vim.o.lazyredraw
  vim.o.lazyredraw = true

  -- 2. window 操作（mount/show/hide など）
...
**適用箇所**:
- `_push_detail_view`: describe, help などの詳細 view への遷移
- `_pop_view_with_lifecycle`: 戻る操作
- `_show_resource_menu`: リソース種別切り替え
- `show_port_forwards`: ポートフォワード一覧表示

**注意点**:
- `lazyredraw` は元の値を保存して必ず復元する
- render 後に `vim.cmd("redraw")` を呼ばないと画面が更新されない
- pop 時は `show()` の前に `render()` を呼ぶことも重要（上記「render 順序」参照）